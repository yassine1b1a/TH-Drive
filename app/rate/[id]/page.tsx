...
export default function RateRidePage() {
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      ...
      // Get the driver's ID from the ride
      const { data: rideWithDriver } = await supabase
        .from('rides')
        .select('driver_id')
        .eq('id', rideId)
        .single()

      if (!rideWithDriver?.driver_id) {
      ...
      const { error: ratingError } = await supabase
        ...
        .insert({
          ride_id: rideId,
          rater_id: user.id,
          rated_id: rideWithDriver.driver_id,
          rating: rating,
        ...
        <Card className="border-2 border-primary/10 shadow-lg">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
              ...
              {ride?.driver && ride.driver[0] && (
                <div className="flex items-center gap-3 p-3 bg-background rounded">
                  <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                    <User className="h-5 w-5 text-primary" />
                  </div>
                  <div>
                    <p className="font-medium">{ride.driver[0].full_name || 'Driver'}</p>
                    <p className="text-sm text-muted-foreground">
                      {ride.driver[0].rating ? `${ride.driver[0].rating.toFixed(1)}/5` : 'No rating yet'}
                    </p>
                  </div>
                </div>
              )}
